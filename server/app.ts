...
import type { LogEntry, Match } from '../packages/domain/types';
import { candidates, draft } from '../packages/rules';
import { CATALOG } from '../packages/content/catalog';
import {nextBotEntry} from './bot';

export async function createApp({dataDir,serveDir,now=Date.now}:{dataDir:string;serveDir?:string;now?:()=>number}){
const app=Fastify({logger:false,bodyLimit:1024*1024});
type Room={match:Match;tokens:[string,string];clock?:RoundClock;admissionKeys?:[string,string];lobby?:Lobby;bot?:{player:1;key:string;nextAt:number};soloKey?:string};
...
// 0.3 changes only the final cost comparison; unfinished 0.2 rounds remain reproducible.
if(room.match.engineVersion==='0.2.0'&&room.match.phase!=='showdown')room.match.engineVersion=ENGINE_VERSION;
...
function projection(room:Room,player:number){playable(room);const view=project(room.match,player);return {...view,...(room.bot?{opponent:{...view.opponent,controller:'ai' as const}}:{}),...(room.clock?{clock:{...room.clock,serverNow:now()}}:{})};}
