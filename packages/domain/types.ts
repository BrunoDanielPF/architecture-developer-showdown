...
export interface Incident { type: string; nodeId: string; detail: string }
...
export interface PlayerView { previousWorld?: World; measurements?: {round:number;telemetry:Partial<Telemetry>}[]; clock?: {key:string;deadline:number;duration:number;serverNow:number}; id: string; version: number; phase: Match['phase']; round: number; player: Omit<Player, 'deck' | 'runtime' | 'telemetry'> & { deckCount: number; telemetry: Partial<Telemetry> }; opponent: { name: string; locked: boolean; controller?: 'ai' }; market: CardInstance[]; marketRemaining: number; scenarioRemaining: number; revealed: Omit<Scenario, 'heat' | 'axis' | 'mutation'>[]; world: World; notices: string[]; showdown?: Showdown }
