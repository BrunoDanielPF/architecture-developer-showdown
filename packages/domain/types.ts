...
export interface PublicHandActivity { seq: number; round: number; count: number; kind: 'gain' | 'spend' | 'prepare' | 'shuffle' }
export interface Match { id: string; seed: string; version: number; phase: 'setup' | 'planning' | 'adjustment' | 'telemetry' | 'showdown'; round: number; players: [Player, Player]; market: CardInstance[]; marketDeck: CardInstance[]; scenarios: Scenario[]; revealed: Scenario[]; worldState: WorldState; log: LogEntry[]; publicHandActivity?: [PublicHandActivity[], PublicHandActivity[]]; showdown?: Showdown; notices: [string[], string[]]; contentVersion: string; engineVersion: string }
...
export interface PlayerView { previousWorld?: World; measurements?: {round:number;telemetry:Partial<Telemetry>}[]; clock?: {key:string;deadline:number;duration:number;serverNow:number}; challenge?: DailyChallengeDefinition; challengeResult?: ChallengeResult; id: string; version: number; phase: Match['phase']; round: number; player: Omit<Player, 'deck' | 'runtime' | 'telemetry'> & { deckCount: number; telemetry: Partial<Telemetry> }; opponent: { name: string; locked: boolean; handCount: number; activity: PublicHandActivity[]; controller?: 'ai' }; market: CardInstance[]; marketRemaining: number; scenarioRemaining: number; revealed: Omit<Scenario, 'heat' | 'axis' | 'mutation'>[]; world: World; notices: string[]; showdown?: Showdown }
